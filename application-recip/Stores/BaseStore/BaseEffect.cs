...
public class BaseEffect<T>(IBaseService<T> _baseService) where T : class
{
    ...
    [EffectMethod]
    public virtual async Task HandleItemCreationAction(CreateItemAction<T> itemCreationAction, IDispatcher dispatcher)
    {
        //var itemCreationResult = await _baseService.CreateAsync(itemCreationAction.ItemToCreate);
        ...
        //dispatcher.Dispatch(new ItemCreationResultAction<T>(createdItem, itemCreationResult.IsSuccess));
        //dispatcher.Dispatch(new SetMessageAction(itemCreationResult.Message, messageType));
    }
